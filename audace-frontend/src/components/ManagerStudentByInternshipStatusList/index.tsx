...
const ManagerStudentByInternshipStatusList = () => {
  const [studentsByInternshipStatus, setStudentsByInternshipStatus] =
    useState<StudentsByInternshipFoundStatus>();
  ...
  const filterStudents = useCallback(
    (text: string, tab: string) => {
      if (!studentsByInternshipStatus) return;

      try {
        const filtered = studentsByInternshipStatus[tab].students.filter(
          (student) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const studentsResponse = await getStudentsByInternshipStatus(
          departmentResponse.data.id!
        );
        ...
        <Col className="d-flex">
          <Col style={{ flex: "1" }}>
            ...
            >
              ...
              <option value="studentsWithAcceptedResponse">
                {t("studentsByInternship.row.statusValues.ACCEPTED")}
              </option>
              <option value="studentsWithRefusedResponse">
